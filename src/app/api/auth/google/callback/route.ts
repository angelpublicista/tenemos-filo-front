...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(`${appUrl}/dashboard/integrations?error=google_auth_denied`);
  }

  if (!code || !state) {
    return NextResponse.redirect(`${appUrl}/dashboard/integrations?error=google_auth_failed`);
  }
  ...
  try {
    ...
    const existing = await getIntegrationsByUser(userId, { type: 'google' });
    ...
    return NextResponse.redirect(`${appUrl}/dashboard/integrations?success=google_connected`);
  } catch {
    return NextResponse.redirect(`${appUrl}/dashboard/integrations?error=google_token_failed`);
  }
}
