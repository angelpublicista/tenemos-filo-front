...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(`${appUrl}/dashboard/integrations?error=microsoft_auth_denied`);
  }

  if (!code || !state) {
    return NextResponse.redirect(`${appUrl}/dashboard/integrations?error=microsoft_auth_failed`);
  }
  ...
  try {
    ...
    if (!tokenResponse.ok || tokens.error) {
      return NextResponse.redirect(`${appUrl}/dashboard/integrations?error=microsoft_token_failed`);
    }

    const expiresAt = new Date(Date.now() + tokens.expires_in * 1000).toISOString();
    const existing = await getIntegrationsByUser(userId, { type: 'outlook' });
    ...
    return NextResponse.redirect(`${appUrl}/dashboard/integrations?success=microsoft_connected`);
  } catch {
    return NextResponse.redirect(`${appUrl}/dashboard/integrations?error=microsoft_token_failed`);
  }
}
